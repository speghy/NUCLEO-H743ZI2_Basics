...
#define UART huart3
#define DMA hdma_usart3_rx

/* Define the Size Here */
#define RxBuf_SIZE 256
#define MainBuf_SIZE 512

uint8_t BUFFER RxBuf[RxBuf_SIZE];
uint8_t BUFFER MainBuf[MainBuf_SIZE];
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
		...
		if (oldPos+Size > MainBuf_SIZE)  // If the current position + new data size is greater than the main buffer
		{
			...
			HAL_UART_Transmit(&huart3, (uint8_t *)MainBuf+oldPos, datatocopy, 1000);
			oldPos = 0;  // point to the start of the buffer
			memcpy ((uint8_t *)MainBuf, (uint8_t *)RxBuf+datatocopy, (Size-datatocopy));  // copy the remaining data
			HAL_UART_Transmit(&huart3, (uint8_t *)MainBuf,(Size-datatocopy), 1000);
		...
		else
		{
			...
			HAL_UART_Transmit(&huart3, (uint8_t *)MainBuf+oldPos, Size, 1000);
			newPos = Size+oldPos;
		}
